...
class TripadvisorSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        cur = my_sql.cursor()
        cur.execute("SELECT id,conversation_id FROM pangting")

        datas = cur.fetchall()
        for data in datas:
            id, conversation_id = data
            url = f"https://www.zhihu.com/consult/conversation/{conversation_id}/archive"
            yield scrapy.Request(url=url, headers=headers, callback=lambda response, id=id: self.parse(response, id))

    def parse(self, response, id):
        soup = BeautifulSoup(response.text)
        texts = soup.find_all(attrs={"class": "Archive-countValue"})

        pangTing = PangTing()
        pangTing['id'] = id
        pangTing['price'], pangTing['pangting_count'], pangTing['zan_count'] = texts[0].text, texts[1].text, texts[2].text
        pangTing['content'] = soup.find(attrs={"class": "Archive-messageContentText"}).text
        pangTing['user'] = soup.find(attrs={"class": "Archive-messager"}).text
        pangTing['time'] = soup.find(attrs={"class": "Archive-messageDate"}).text
        yield pangTing