...
if __name__ =="__main__":
    ...
    url = "https://www.zhihu.com/api/v4/infinity/topics?limit=10"
    response = getHTMLText(url)
    jsonBody = json.loads(response.text.encode('utf-8'))
    datas = jsonBody['data']

    sql = "INSERT INTO answer_column(answerer_count,column_id,name) VALUES(%s,%s,%s)"
    for data in datas:
        answerer_count = data['answerer_count']
        column_id = data['id']
        name = data['name']
        cursor.execute(sql, [str(answerer_count), column_id, name])
